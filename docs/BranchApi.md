...
# **merge**
> Task merge(branch_id, destination_branch_id)
...
with gridly.ApiClient(configuration) as api_client:
    ...
    try:
        # merge
        api_response = api_instance.merge(branch_id, destination_branch_id)
        pprint(api_response)
    except gridly.ApiException as e:
    ...
    try:
        # merge
        api_response = api_instance.merge(branch_id, destination_branch_id, merge_record_options=merge_record_options)
        pprint(api_response)
    except gridly.ApiException as e:
        print("Exception when calling BranchApi->merge: %s\n" % e)
```
